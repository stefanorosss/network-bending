...
def generate(args, g_ema, device, mean_latent, t_dict_list):
    with torch.no_grad():
        ...
        # print(t_dict_list['params'][0], type(t_dict_list['params'][0]))
        paramX = np.linspace(0,t_dict_list['params'][0],int(args.frames) )
        paramY = np.linspace(0,t_dict_list['params'][1], int(args.frames) )
        paramXY =np.c_[paramX,paramY]
        
        for _layer in tqdm(range(int(args.layers))):
            layerPath = f'sample/layer{_layer}'
            if not os.path.exists(layerPath):
                os.makedirs(layerPath)

            t_dict_list['layerID'] = _layer

            for i, _param in enumerate(paramXY):
                
                t_dict_list['params'] = _param
                ...
                utils.save_image(
                    sample,
                    f'{layerPath}/{str(i).zfill(6)}.png',
                    nrow=1,
                    normalize=True,
                    range=(-1, 1))
            cmd = f"ffmpeg -y -r {16} -i {layerPath}/%06d.png -vcodec libx264 -pix_fmt yuv420p {layerPath}/sample_vid.mp4"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--frames', type=str, default='80')
    ...
    if args.load_latent == "":
        generate(args, g_ema, device, mean_latent, transform_dict_list[0])
    else:
